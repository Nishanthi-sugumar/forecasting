library(readxl)
Airlines<-read_xlsx(file.choose()) # read the Amtrack data
...
l<-cbind(Airlines,X)
View(l)
colnames(l)[2]<-"Passengers"
colnames(l)
l["t"]<- 1:96
View(l)
l["log_rider"]<-log(l["Passengers"])
l["t_square"]<-l["t"]*l["t"]
attach(l)
...
new_model <- lm(Passengers~t+t_square+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov,data=l)
...
####################### Predicting new data #############################
library(readxl)
test_data<-read_xlsx(file.choose(),1)
View(test_data)
pred_new<-data.frame(predict(new_model,newdata=test_data,interval = 'predict'))
View(pred_new)
Final <- as.data.frame(cbind(Month,test_data$Passengers,new_model_fin))
