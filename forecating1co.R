library(readxl)
coco<-read_xlsx(file.choose())

View(coco) # Seasonality 4 quaterly
windows()
plot(coco$Sales,type="o")
# So creating 4 dummy variables 

Q1 <-  ifelse(grepl("Q1",coco$Quarter),'1','0')

Q2 <-  ifelse(grepl("Q2",coco$Quarter),'1','0')
Q3 <-  ifelse(grepl("Q3",coco$Quarter),'1','0')
Q4 <-  ifelse(grepl("Q4",coco$Quarter),'1','0')


y<-cbind(coco,Q1,Q2,Q3,Q4)
View(y)
colnames(y)[2]<-"sales"
colnames(y)
y["t"]<- 1:42
View(y)
y["log_rider"]<-log(y["sales"])
y["t_square"]<-y["t"]*y["t"]
attach(y)
...
new_model <- lm(sales~t+t_square+Q1+Q2+Q3+Q4,data=y)
...
pred_res$pred
acf(k$residuals)
write.csv(y,file="y.csv",col.names = F,row.names = F)
...
View(pred_new)
Quarter <- as.data.frame(test_data$Quarter)

Final <- as.data.frame(cbind(Quarter,test_data$Sales,pred_new))
